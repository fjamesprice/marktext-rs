...
| `block-kinds.md` | ~1.6 KB | Generated: hand-written, **added at M3 S1** | The five `mt_doc::Block` variants no other corpus file contains: **front matter** (which must be the first bytes of a file, so this one opens with it), **setext headings** at both levels, an **HTML block**, all five **diagram** kinds, and a **footnote** definition. It exists because M3's layout goldens are M3-R6's instrument — *"the first golden for each block kind is read by eye once"* — and without it five of the nineteen kinds appeared in no golden at all, three of them being exactly the kinds M3 §5 D11 decides. Pure ASCII, deliberately: the vendored CJK face is a `pyftsubset` derived from this directory (`assets/fonts/README.md` §6). Small enough to read beside the golden it produces. **A `Footnote` block needs `Options::footnote`, which `MUYA_DEFAULT` sets to `false` because muya does** — so `cargo xtask layout` parses this file, and only this file, with the extension on and records that in the golden's `parse` header line. Every other harness reads it at `MUYA_DEFAULT` like all the others, which makes the `[^why]:` line a paragraph to them. |
