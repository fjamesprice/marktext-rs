...
//! lands in? This is the **git `main`** arm; `e1-parley-0-11` is the same file
//! against the released 0.11.0, and the diff between the two files is itself an
//! answer (secondary question 2, "breaking churn").
//!
...
const ARM: &str = "parley git main @ a0752c7bdc3ad88dac19fc194d2b8e57e59bea2e (2026-08-10)";
...
fn run_case(font_cx: &mut FontContext, layout_cx: &mut LayoutContext<()>, case: &Case) -> bool {
    ...
    builder.push_inline_box(InlineBox {
        ...
        width: BOX_W,
        height: BOX_H,
        // PR #639. `None` reproduces 0.11.0's only behaviour (bottom edge on
        // the text baseline), which is what the bidi cases hold constant.
        baseline: None,
    });
                ...
                PositionedLayoutItem::InlineBox(b) => {
                    items.push(Item {
                        ...
                        kind: Kind::Box {
                            ...
                            y: b.y,
                            h: b.height,
                            // PR #639 added this field; the 0.11.0 arm
                            // hard-codes `None` because it does not exist.
                            baseline: b.baseline,
                        },
                    });
                }
...
fn baseline_probe(font_cx: &mut FontContext, layout_cx: &mut LayoutContext<()>) {
    ...
    // Four boxes of identical geometry, differing only in `baseline`. If #639
    // works, `None` pins the box's bottom edge to the text baseline, and a
    // `Some(b)` puts the box's own baseline — `b` pixels below its top — on the
    // text baseline, so `y + b` must equal the line's baseline every time.
    for baseline in [None, Some(BOX_H), Some(BOX_H / 2.0), Some(0.0)] {
        let mut builder = layout_cx.ranged_builder(font_cx, TEXT, 1.0, true);
        builder.push_default(StyleProperty::FontSize(FONT_SIZE));
        builder.push_inline_box(InlineBox {
            id: 7,
        ...
        println!("  --- InlineBox::baseline = {baseline:?}");
        println!(
            "      Layout::first_baseline() = {:?}, Layout::last_baseline() = {:?}",
            layout.first_baseline(),
            layout.last_baseline()
        );
        for line in layout.lines() {
            println!(
            ...
            for item in line.items() {
                if let PositionedLayoutItem::InlineBox(b) = item {
                    println!(
                        "      box: x {:.4} y {:.4} w {:.4} h {:.4} baseline {:?}; bottom edge {:.4}; box baseline in layout coords {:?}",
                        b.x,
                        b.y,
                        b.width,
                        b.height,
