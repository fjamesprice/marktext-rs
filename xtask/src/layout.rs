...
const FORMAT_VERSION: &str = "layout-golden v1";
...
pub fn main(repo_root: &Path, args: &[String]) -> Result<i32, String> {
    ...
    for (name, text) in &documents {
        let parsed = mt_md::parse(text, mt_md::Options::MUYA_DEFAULT);
        for theme in &themes {
            ...
            let body = serialize(&provenance, theme, name, text.len(), &list, detail, &fonts);
    ...
    if !uncovered.is_empty() && opts.only.is_none() {
        println!(
...
mod tests {
    ...
    fn every_committed_golden_has_the_declared_header() {
        ...
        for entry in std::fs::read_dir(&dir).expect("bench/layout-goldens") {
            ...
            assert_eq!(lines.len(), 10, "{name}: header is ten fields");
            for (i, key) in [
                "theme",
                "theme-width",
                "content-width",
    ...
    /// **M3-R6's first-review clause, ratcheted.**
    ///
    /// The risk register says the first golden for each block kind is read by
    /// eye once and the review is recorded. Five of the nineteen kinds have no
    /// first golden to read, because `bench/corpus/` contains none of them:
    ///
    /// | kind | why the corpus has none |
    /// |---|---|
    /// | `setext-heading` | the generator writes `## Section n`; nothing is underlined with `===` |
    /// | `html-block` | no raw HTML in any corpus file |
    /// | `frontmatter` | `front_matter: true` is on in `Options::MUYA_DEFAULT`, but no file opens with `---` |
    /// | `diagram` | no fence carries a `mermaid` / `vega-lite` / `flowchart` info string |
    /// | `footnote` | **`Options::MUYA_DEFAULT` sets `footnote: false`** — muya's own default — so `10kb.md`'s `[^why]:` line parses as a paragraph, exactly as `bench/corpus/README.md` says it does |
    ///
    /// Three of those five are D11's arms and the fourth cost six new theme
    /// fields and a drawn label at S1, so the gap is not academic.
    ///
    /// This test fails **in both directions**, which is `spec/expected-failures.json`'s
    /// shape: a kind that stops being exercised is a corpus regression, and a
    /// kind that *starts* being exercised is a golden nobody has read yet.
    /// Either way, look, then edit the list in the same commit.
    #[test]
    fn the_kinds_no_golden_exercises_are_exactly_the_five_the_corpus_lacks() {
        const UNEXERCISED_KINDS: [&str; 5] = [
            "setext-heading",
            "html-block",
            "frontmatter",
            "diagram",
